...
public class MedianOfTwoSortedArrays {
	...
	public double helper(int A[], int astart, int aend, int B[], int bstart,
			int bend, int k) {
		int m = aend - astart + 1;
		int n = bend - bstart + 1;
		if (m > n)
		...
		if (A[astart + partA - 1] < B[bstart + partB - 1])
			return helper(A, astart + partA, aend, B, bstart, bstart + partB, k
					- partA);
		else if (A[astart + partA - 1] > B[bstart + partB - 1])
			return helper(A, astart, astart + partA, B, bstart + partB, bend, k
					- partB);
		else
